...
source_dir = 'X:\000_Data\SEEG\data';
...
soz_label_file = 'X:\000_Data\SEEG\labels\all_pats_bipole_ez_labels.csv';
metric_loc_in_struct = 3;

%Path to violin plot folder
addpath('Violinplot-Matlab-master')
% Path to EDFread
addpath('X:\Jared\matlab_toolboxes\edfRead')
% Path to fieldtrip toolbox
addpath('X:\Jared\matlab_toolboxes\fieldtrip-20190819');
% Path to RolstonCode for Imaginary COherence
addpath(genpath('RolstonCode'));
...
for i = 1:length(pat_dir_list)% 1 to the amount of selected patients
    ...
    all_T = nan(length(edf_idxs),length(hdrs{1}.label),length(hdrs{1}.label));
    all_A = all_T;
    all_B = all_T;
    all_G_low = all_T;
    all_G_high = all_T;
    ...
    for j = 1:length(edf_idxs)
        ...
        switch metric
            ...
            case 'PDC'
                [T,A,B,G_low,G_high] = pdc_calc(filt_data,hdrs{j},mvar_order,long_epoch_seg_length);
...
roc_bip_in = cell(1,5);
roc_bip_out = cell(1,5);
roc_soz_label = cell(1,5);
