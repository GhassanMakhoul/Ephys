...
for i in range(len(pat_epoch_folders)):
    ...
    tmp_file_types = confident_sleep_categories
...
# for p in pat_order:
    ...
    for i in range(tot_nights):
        ...
        file_size_arr = np.zeros(len(fnames_to_include))
        for j in range(len(fnames_to_include)):
            ...
            file_size_arr[j] = os.path.getsize(edf_source_fpath)
        ...
        break
    pat_file_size_list.append(np.amax(file_size_arr))
    # except:
    #     with open(log_fpath, 'a') as f:
    #        f.write("ERROR WITH PatientName: " + pat_ids[p] + "\n")
    
pat_file_size_arr = np.asarray(pat_file_size_list)

pat_order = np.argsort(pat_file_size_arr)
