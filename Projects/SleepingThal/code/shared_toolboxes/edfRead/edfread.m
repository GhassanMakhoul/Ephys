...
hdr.patientID  = char(fread(fid,80)'); %fread(fid,80,'*char')';
hdr.recordID   = char(fread(fid,80)'); %fread(fid,80,'*char')';
hdr.startdate  = char(fread(fid,8)'); %fread(fid,8,'*char')';% (dd.mm.yy)
% hdr.startdate  = datestr(datenum(fread(fid,8,'*char')','dd.mm.yy'), 29); %'yyyy-mm-dd' (ISO 8601)
hdr.starttime  = char(fread(fid,8)'); %fread(fid,8,'*char')';% (hh.mm.ss)
% hdr.starttime  = datestr(datenum(fread(fid,8,'*char')','hh.mm.ss'), 13); %'HH:MM:SS' (ISO 8601)
hdr.bytes      = str2double(char(fread(fid,8)')); %str2double(fread(fid,8,'*char')');
reserved       = fread(fid,44);%#ok
hdr.records    = str2double(char(fread(fid,8)')); %str2double(fread(fid,8,'*char')');
if hdr.records == -1
	beep
...
hdr.duration   = str2double(char(fread(fid,8)')); %str2double(fread(fid,8,'*char')');
% Number of signals
hdr.ns    = str2double(char(fread(fid,4)')); %str2double(fread(fid,4,'*char')');
for ii = 1:hdr.ns
    hdr.label{ii} = regexprep(char(fread(fid,16)'),'\W',''); %regexprep(fread(fid,16,'*char')','\W','');
end
...
for ii = 1:hdr.ns
    hdr.transducer{ii} = char(fread(fid,80)'); %fread(fid,80,'*char')';
end
% Physical dimension
for ii = 1:hdr.ns
    hdr.units{ii} = char(fread(fid,8)'); %fread(fid,8,'*char')';
end
% Physical minimum
for ii = 1:hdr.ns
    hdr.physicalMin(ii) =  str2double(char(fread(fid,8)')); %str2double(fread(fid,8,'*char')');
end
% Physical maximum
for ii = 1:hdr.ns
    hdr.physicalMax(ii) =  str2double(char(fread(fid,8)')); %str2double(fread(fid,8,'*char')');
end
% Digital minimum
for ii = 1:hdr.ns
    hdr.digitalMin(ii) =  str2double(char(fread(fid,8)')); %str2double(fread(fid,8,'*char')');
end
% Digital maximum
for ii = 1:hdr.ns
    hdr.digitalMax(ii) =  str2double(char(fread(fid,8)')); %str2double(fread(fid,8,'*char')');
end
for ii = 1:hdr.ns
    hdr.prefilter{ii} = char(fread(fid,80)'); %fread(fid,80,'*char')';
end
for ii = 1:hdr.ns
    hdr.samples(ii) = str2double(char(fread(fid,8)')); %str2double(fread(fid,8,'*char')');
end
for ii = 1:hdr.ns
    reserved    = char(fread(fid,32)'); %fread(fid,32,'*char')';%#ok
end
